...
// C263BannerStyle banner样式配置
type C263BannerStyle struct {
...
// C263MoreFrameStyle 多帧样式配置
type C263MoreFrameStyle struct {
	Ratio        float64 `json:"ratio,omitempty"`         // 比例
	LeftMargin   int     `json:"left_margin,omitempty"`   // 左边距
	RightMargin  int     `json:"right_margin,omitempty"`  // 右边距
	TopMargin    int     `json:"top_margin,omitempty"`    // 上边距
...
type C263 struct {
	...
	BannerStyle       *C263BannerStyle    `json:"banner_style,omitempty"`        // banner样式
	MoreFrameStyle    *C263MoreFrameStyle `json:"more_frame_style,omitempty"`    // 多帧样式
	MoreFrame         *C263MoreFrame      `json:"more_frame,omitempty"`          // 多帧内容
	Banner            []*C263Banner       `json:"banner,omitempty"`              // banner列表
	GridColumn        int                 `json:"grid_column,omitempty"`         // 宫格列数
	Grid              []*C263Grid         `json:"grid,omitempty"`                // 宫格列表
	OpenUrl           string              `json:"openurl,omitempty"`             // 打开链接
}

// NewCard263 创建Card263实例
func NewCard263() *C263 {
	return &C263{BaseItem: BaseItem{Base: Base{CardType: 263}}}
}
