...
// C281TopContent 内部卡片上半部分内容
type C281TopContent struct {
...
// C281BottomContent 内部卡片下半部分内容
type C281BottomContent struct {
...
// C281Item 内部卡片项
type C281Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C281TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C281BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C281 struct {
	...
	Items                []*C281Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard281 创建Card281实例
func NewCard281() *C281 {
	return &C281{BaseItem: BaseItem{Base: Base{CardType: 281}}}
}
