...
// C282TopContent 内部卡片上半部分内容
type C282TopContent struct {
...
// C282BottomContent 内部卡片下半部分内容
type C282BottomContent struct {
...
// C282Item 内部卡片项
type C282Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C282TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C282BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C282 struct {
	...
	Items                []*C282Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard282 创建Card282实例
func NewCard282() *C282 {
	return &C282{BaseItem: BaseItem{Base: Base{CardType: 282}}}
}
