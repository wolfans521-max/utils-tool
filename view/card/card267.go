...
// C267TopContent 内部卡片上半部分内容
type C267TopContent struct {
...
// C267BottomContent 内部卡片下半部分内容
type C267BottomContent struct {
...
// C267Item 内部卡片项
type C267Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C267TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C267BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C267 struct {
	...
	Items                []*C267Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard267 创建Card267实例
func NewCard267() *C267 {
	return &C267{BaseItem: BaseItem{Base: Base{CardType: 267}}}
}
