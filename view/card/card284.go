...
// C284TopContent 内部卡片上半部分内容
type C284TopContent struct {
...
// C284BottomContent 内部卡片下半部分内容
type C284BottomContent struct {
...
// C284Item 内部卡片项
type C284Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C284TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C284BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C284 struct {
	...
	Items                []*C284Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard284 创建Card284实例
func NewCard284() *C284 {
	return &C284{BaseItem: BaseItem{Base: Base{CardType: 284}}}
}
