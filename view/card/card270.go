...
// C270TopContent 内部卡片上半部分内容
type C270TopContent struct {
...
// C270BottomContent 内部卡片下半部分内容
type C270BottomContent struct {
...
// C270Item 内部卡片项
type C270Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C270TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C270BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C270 struct {
	...
	Items                []*C270Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard270 创建Card270实例
func NewCard270() *C270 {
	return &C270{BaseItem: BaseItem{Base: Base{CardType: 270}}}
}
