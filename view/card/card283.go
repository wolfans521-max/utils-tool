...
// C283TopContent 内部卡片上半部分内容
type C283TopContent struct {
...
// C283BottomContent 内部卡片下半部分内容
type C283BottomContent struct {
...
// C283Item 内部卡片项
type C283Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C283TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C283BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C283 struct {
	...
	Items                []*C283Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard283 创建Card283实例
func NewCard283() *C283 {
	return &C283{BaseItem: BaseItem{Base: Base{CardType: 283}}}
}
