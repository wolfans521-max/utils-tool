...
// C265TopContent 内部卡片上半部分内容
type C265TopContent struct {
...
// C265BottomContent 内部卡片下半部分内容
type C265BottomContent struct {
...
// C265Item 内部卡片项
type C265Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C265TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C265BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C265 struct {
	...
	Items                []*C265Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard265 创建Card265实例
func NewCard265() *C265 {
	return &C265{BaseItem: BaseItem{Base: Base{CardType: 265}}}
}
