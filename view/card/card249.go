...
// C249BannerStyle banner样式配置
type C249BannerStyle struct {
...
// C249MoreFrameStyle 多帧样式配置
type C249MoreFrameStyle struct {
	Ratio        float64 `json:"ratio,omitempty"`         // 比例
	LeftMargin   int     `json:"left_margin,omitempty"`   // 左边距
	RightMargin  int     `json:"right_margin,omitempty"`  // 右边距
	TopMargin    int     `json:"top_margin,omitempty"`    // 上边距
...
type C249 struct {
	...
	BannerStyle       *C249BannerStyle    `json:"banner_style,omitempty"`        // banner样式
	MoreFrameStyle    *C249MoreFrameStyle `json:"more_frame_style,omitempty"`    // 多帧样式
	MoreFrame         *C249MoreFrame      `json:"more_frame,omitempty"`          // 多帧内容
	Banner            []*C249Banner       `json:"banner,omitempty"`              // banner列表
	GridColumn        int                 `json:"grid_column,omitempty"`         // 宫格列数
	Grid              []*C249Grid         `json:"grid,omitempty"`                // 宫格列表
	OpenUrl           string              `json:"openurl,omitempty"`             // 打开链接
}

// NewCard249 创建Card249实例
func NewCard249() *C249 {
	return &C249{BaseItem: BaseItem{Base: Base{CardType: 249}}}
}
