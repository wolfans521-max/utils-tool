...
// C271TopContent 内部卡片上半部分内容
type C271TopContent struct {
...
// C271BottomContent 内部卡片下半部分内容
type C271BottomContent struct {
...
// C271Item 内部卡片项
type C271Item struct {
	Scheme            string             `json:"scheme,omitempty"`             // 跳转scheme
	TopContent        *C271TopContent    `json:"top_content,omitempty"`        // 上半部分内容
	BottomContent     *C271BottomContent `json:"bottom_content,omitempty"`     // 下半部分内容
...
type C271 struct {
	...
	Items                []*C271Item `json:"items,omitempty"`                   // 内部卡片列表
}

// NewCard271 创建Card271实例
func NewCard271() *C271 {
	return &C271{BaseItem: BaseItem{Base: Base{CardType: 271}}}
}
