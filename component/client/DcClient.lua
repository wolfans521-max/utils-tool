...
function _M:s_curl(opt)
    ...
    local port  = arrayHelper.get_value(opt, 'args.port', 0)
...
function _M:handle_request(opt)
    ...
    _dc:set_timeouts(timeout[1],timeout[2],timeout[3])

    local args = json_encode(opt.args)
    return _dc:request(node.host, node.port, args, opt.keepalive)
end
